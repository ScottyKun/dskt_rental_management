...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 to-gray-100">
    <form action="{{ route('immeubles.update', $immeuble->id) }}" method="POST"
        ...
        <h2 class="text-3xl font-bold text-center text-gray-800">
            <i class="fa-solid fa-building text-blue-500 mr-2"></i>Modifier Immeuble
        </h2>

        {{-- Nom --}}
        <div>
            <label class="block mb-1 font-semibold">Nom</label>
            <input type="text" name="name" value="{{ old('name', $immeuble->name) }}"
        ...
        <div>
            ...
            <input type="text" name="address" value="{{ old('address', $immeuble->address) }}"
                class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none" required>
        </div>
        ...
        <div>
            ...
            <input type="text" name="town" value="{{ old('town', $immeuble->town) }}"
                class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none" required>
        </div>
        ...
        <div>
            ...
            <textarea name="description"
                class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none">{{ old('description', $immeuble->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <select name="manager_id" class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none" required>
                @foreach($managers as $manager)
                    <option value="{{ $manager->id }}" {{ $manager->id === $immeuble->manager_id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="number" name="nb_apartments" value="{{ old('nb_apartments', $immeuble->nb_apartments) }}"
        ...
        <div>
            ...
            <select name="status" class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none">
                <option value="actif" {{ $immeuble->status === 'actif' ? 'selected' : '' }}>Actif</option>
                <option value="inactif" {{ $immeuble->status === 'inactif' ? 'selected' : '' }}>Inactif</option>
            </select>
        </div>
