...
@section('title', 'Modifier un Appartement')
...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 to-gray-100">
    <form action="{{ route('appartements.update', $appartement->id) }}" method="POST"
        ...
        <h2 class="text-3xl font-bold text-center text-gray-800">
            <i class="fa-solid fa-building text-blue-500 mr-2"></i>Modifier un Appartement
        </h2>

        {{-- Nom --}}
        <div>
            <label class="block mb-1 font-semibold">Nom</label>
            <input type="text" name="name" value="{{ old('name', $appartement->name) }}"
        ...
        <div>
            ...
            <textarea name="description"
                      class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none">{{ old('description', $appartement->description) }}</textarea>
        </div>

        {{-- Type --}}
        <div>
            <label class="block mb-1 font-semibold">Type</label>
            <input type="text" name="type" value="{{ old('type', $appartement->type) }}"
                   class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none" required>
        </div>
        ...
        <div>
            ...
            <input type="number" name="area" value="{{ old('area', $appartement->area) }}"
                   class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none" required>
        </div>
        ...
        <div>
            ...
            <input type="number" step="0.01" name="rent" value="{{ old('loyer', $appartement->rent) }}"
                   class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none" required>
        </div>
        ...
        <div>
            ...
            <select name="status" class="w-full p-3 border rounded focus:ring-2 focus:ring-blue-500 outline-none">
                @php $status = old('status', $appartement->status); @endphp
                <option value="disponible" {{ $status === 'disponible' ? 'selected' : '' }}>Disponible</option>
                <option value="occupe" {{ $status === 'occupe' ? 'selected' : '' }}>Occupé</option>
                <option value="en_renovation" {{ $status === 'en_renovation' ? 'selected' : '' }}>En rénovation</option>
            </select>
        </div>
